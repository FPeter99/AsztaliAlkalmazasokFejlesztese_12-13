...
namespace Nyelviskola_Lib
{
    public class DataStore
    {
        ...
        private DataStore() 
        {
            ...
            tanitasiAlkalmak = File.ReadAllLines("Input\\tanitasi_alkalom.csv").Skip(1).Select(x => new TanitasiAlkalom(x)).ToList();
        ...
        static void ExportToJson<T>(List<T> elemek, string fajlNev) 
        {


            File.WriteAllText(fajlNev, JsonSerializer
                .Serialize(elemek, new JsonSerializerOptions { WriteIndented = true }));
        }
